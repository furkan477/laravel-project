                 ...
                 <thead>
                  ...
                  @foreach ($flight as $fly)
                    @if (auth()->check() && auth()->user()->id == $fly->user_id)
                     <tr>
                      <td><p>{{ $fly->id }}</p></td>
                      <td><p>{{ $fly->nereden }}</p></td>
                      <td><p>{{ $fly->nereye }}</p></td>
                      <td><p>{{ $fly->gidis_tarihi }}</p></td>
                      <td><p>{{ $fly->gelis_tarihi }}</p></td>
                      <td><p>{{ $fly->status }}</p></td>
                      <td><p>{{ $fly->progress }}</p></td>
                    

                      <td>
                        <div class="btn-group btn-group-md">
                          <button class="btn btn-md btn-info">Tamamlandı</button>
                          <a href="{{route('delete',['id' => $fly->id])}}" class="btn btn-md btn-danger">Sil</a>
                          <a href="{{route('flightUpdate_')}}" class="btn btn-md btn-warning">Güncelle</a>
                        </div>
                      </td>
                     </tr>
                    @endif
