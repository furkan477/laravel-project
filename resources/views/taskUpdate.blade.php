...
<div class="wrapper">
    <div class="content-wrapper">
        ...
        <div class="content">
             ...
             <form action="{{ route('taskUpdate', ['task' => $task->id])}}" method="post">
                @csrf
                <div class="card-body">
                    <div class="form-group">
                        <label>Görev Türü</label>
                        <input type="text" class="form-control" name="task_type" value="{{ $task->task_type }}">
                    </div>
                    <div class="form-group">
                        <label>Görev Açıklaması</label>
                        <input type="text" class="form-control" name="task_description" value="{{ $task->task_description }}"></input>
                    </div>
                    <div class="form-group">
                        <label>Görev Başlangıç Tarihi</label>
                        <input type="date" class="form-control" name="task_start_date" value="{{ $task->task_start_date }}">
                    </div>
                    <div class="form-group">
                        <label>Görev Bitiş Tarihi (isteğe bağlı)</label>
                        <input type="date" class="form-control" name="task_end_date" value="{{ $task->task_end_date }}">
                    </div>
                    <div class="form-group">
                        <label>Görev İlerleyiş</label>
                        <input type="range" min="0" max="100" class="form-control" name="task_progress" value="{{ $task->task_progress }}">
                    </div>
                    <div class="form-group">
                        <label>Görev Durum</label>
                        <select name="task_status" class="form-control" value="{{ $task->task_status }}">
